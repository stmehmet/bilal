...
1. **Tailnet membership as the outermost firewall** — the Pi's Tailscale IP is unreachable from the public internet. Only devices logged into the maintainer's tailnet can resolve `bilal-<machine-id>` or reach `100.x.y.z`.
